...
public class UserRatingMap
{
    ...
    public UserRatingMap(JSONObject data)
    {
        if (data.type != JSONObject.Type.OBJECT)
        {
            throw new Exception("UserRatingMap::UserRatingMap unkown data format");
        }
        this.map_name = data.GetField("n").str;
        this.map = MapList.Instance[this.map_name];
    ...
    public UserRatingMap(Dictionary<string, object> data)
    {
        this.map_name = ((!data.ContainsKey("n")) ? "null" : data["n"].ToString());
        this.map = MapList.Instance[this.map_name];
