...
public class MomentChainLightning : MonoBehaviour
{
    ...
    public void Launch(long launchDelay, GameObject hitSpark)
    {
        this.launchTime = TimeManager.Instance.NetworkTime + launchDelay;
        this.hitSpark = hitSpark;
    }

    private void LateUpdate()
    {
        if (TimeManager.Instance.NetworkTime >= this.launchTime && this.launchTime != 0L && !this.active)
        {
            this.active = true;
            if ((Object)this.hitSpark != (Object)null)
            {
                GameObject gameObject = UnityEngine.Object.Instantiate(this.hitSpark);
                gameObject.transform.position = this.target.position;
                gameObject.transform.LookAt(gameObject.transform.position + new Vector3(0f, 1f, 0f));
            }
        }
        if (!this.active)
        {
        ...
        else
        {
            ...
            {
                base.transform.position = this.origin.transform.position;
            }
