...
public class FractalNoise
{
    ...
    public FractalNoise(float inH, float inLacunarity, float inOctaves, Perlin noise)
    {
        ...
        {
            this.m_Exponent[i] = (float)Math.Pow((double)this.m_Lacunarity, (double)(0f - inH));
    ...
    public float BrownianMotion(float x, float y)
    {
        float num = 0f;
        long num2;
        for (num2 = 0L; num2 < this.m_IntOctaves; num2++)
        {
            num = this.m_Noise.Noise(x, y) * this.m_Exponent[num2];
            x *= this.m_Lacunarity;
            y *= this.m_Lacunarity;
        }
        float num3 = this.m_Octaves - (float)this.m_IntOctaves;
        return num + num3 * this.m_Noise.Noise(x, y) * this.m_Exponent[num2];
    }
}
